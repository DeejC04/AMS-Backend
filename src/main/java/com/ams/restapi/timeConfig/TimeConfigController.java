...
class TimeConfigController {
    ...
    String method(@RequestParam("stuName") String stuName) {
        return "Hi " + stuName + ". Nice to meet you!";
    }

    @PostMapping("/timeConfig/{courseID}")
    ...
    List<TimeConfig> search(@PathVariable Long courseID) {
        return timerepo.findByCourseID(courseID);
    }
